...
from iib.workers.tasks.containerized_utils import cleanup_on_failure, write_build_metadata
...
def handle_containerized_rm_request(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f'iib-{request_id}-') as temp_dir:
            ...
            branch = "main"  # TODO: Use ocp_version when branches are created per version
            ...
            # ===================================================================
            # ImageStream cache operations - COMMENTED OUT FOR DEV ENV
            # Uncomment for production use with OpenShift cluster
            # ===================================================================
            ...
            # DEV MODE: Pull directly from Quay without ImageStream cache
            log.info('Pulling index.db artifact directly from Quay (dev mode)')
            artifact_ref = get_indexdb_artifact_pullspec(from_index)
            artifact_dir = get_oras_artifact(
                artifact_ref,
                temp_dir,
            ...
            try:
                wait_for_pipeline_completion(pipelinerun_name)
                ...
                raise IIBError(f"Konflux pipeline failed: {e}")

            # Extract IMAGE_URL from pipelinerun results
            image_url = get_pipelinerun_image_url(pipelinerun_name)
            ...
            for spec in output_pull_specs:
                try:
                    ...
                    log.error("Failed to copy index to %s: %s", spec, e)
                    cleanup_on_failure(
                        mr_details=mr_details,
                        index_git_repo=index_git_repo,
                        request_id=request_id,
            ...
            if overwrite_from_index_token and operators_in_db and index_db_path and os.path.exists(index_db_path):
                set_request_state(request_id, 'in_progress', 'Pushing updated index database')
                artifact_ref = get_indexdb_artifact_pullspec(from_index)

                # Convert to relative path to avoid collisions when extracting
                relative_index_db_path = os.path.relpath(index_db_path)

                # Push with v4.x floating tag
                push_oras_artifact(
                    artifact_ref=artifact_ref,
                    local_path=relative_index_db_path,
                    annotations={
                ...
                log.info('Pushed updated index.db %s to registry', artifact_ref)
            ...
            push_oras_artifact(
                artifact_ref=request_artifact_ref,
                local_path=relative_index_db_path,
                annotations={
                    'request_id': str(request_id),
                    'operation': 'remove_operators',
            ...
            log.info('Pushed %s to registry', request_artifact_ref)
