...
def patch_request(request_id):
    ...
    for key, value in payload.items():
        if key == 'arches':
            ...
            if not isinstance(value, list):
                raise ValidationError(f'The value for "{key}" must be a list of non-empty strings')
            for bundle in value:
                if not isinstance(bundle, str):
                    raise ValidationError(
                        f'The list of values for "{key}" must be non-empty string'
                    )
    ...
    if payload.get('recursive_related_bundles', []):
        recursive_related_request = Request.query.get_or_404(request_id)
        for bundle in payload.get('recursive_related_bundles', []):
            recursive_related_request.add_related_image(bundle)
...
def get_nested_bundles(request_id):
    ...
    s3_bucket_name = flask.current_app.config['IIB_AWS_S3_BUCKET_NAME']
    if not s3_bucket_name and not recursive_related_bundles_dir:
        raise NotFound()
